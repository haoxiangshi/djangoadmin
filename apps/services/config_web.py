...
def getConfigInfo():
    config_list = []
    # 查询配置列表
    configList = Config.query.filter(Config.is_delete == 0).order_by(Config.sort.asc()).all()
    if configList:
        # 遍历数据源
        for val in configList:
            ...
            # 查询配置数据列表
            dataList = ConfigData.query.filter(
                and_(ConfigData.is_delete == 0, ConfigData.status == 1, ConfigData.config_id == val.id)).order_by(
...
def saveConfigInfo():
    ...
    if dict_data:
        for key in dict_data:
            ...
            # 根据编码查询配置项
            config_data = ConfigData.query.filter(and_(ConfigData.is_delete == 0, ConfigData.code == key)).first()
            if not config_data:
                continue
            # 设置
