...
class AdSortForm(FlaskForm):
    ...
    name = StringField(
        label='广告位名称',
        validators=[
            DataRequired(message='广告位名称不能为空'),
            Length(max=255, message='广告位名称长度不得超过255个字符')
        ]
    )
    # 站点ID
    ...
    loc_id = IntegerField(
        label='广告位置',
        validators=[
            DataRequired(message='广告位置不能为空'),
            NumberRange(min=0, message='广告位置值不得小于0')
        ]
    )
    ...
    description = StringField(
        label='广告位描述',
        validators=[
            DataRequired(message='广告位描述不能为空'),
            Length(max=255, message='广告位描述长度不得超过255个字符')
        ]
    )
    # 广告位排序
    sort = IntegerField(
        label='广告位排序',
        validators=[
            DataRequired(message='广告位排序不能为空'),
            NumberRange(min=0, max=99999, message='广告位排序值在0~99999之间')
        ]
    )
